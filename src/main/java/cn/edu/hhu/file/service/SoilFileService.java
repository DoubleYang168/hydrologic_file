...
@Service
public class SoilFileService {


    @Value("${hehai.file.soil.rootdir}")
    private String soilRootDirectory;
    ...
    public List<FileDTO> listFile(final String dir) {
        ...
        if (this.soilHanjiangLocation.equals(dir) || this.soilHuaiheLocation.equals(dir) || this.soilHuaihebengbuLocation.equals(dir) || this.soilHeiheLocation.equals(dir) || this.soilZiyaheLocation.equals(dir)) {
            final String dst = Paths.get(this.rootLocation, this.soilRootDirectory, dir).toString();
...
//        fileDTOList.sort(Comparator.comparing((Function<? super Object, ? extends Comparable>)FileDTO::getLastModified).reversed());
    ...
    public InputStream downloadFiles(final String dir, final List<String> names) throws IOException {
        if (names.size() <= 1) {
            final String fileDst = Paths.get(this.rootLocation, this.soilRootDirectory, dir, names.get(0)).toString();
            final File file = new File(fileDst);
            return new FileInputStream(file);
        }
        final List<File> fileList = new ArrayList<File>();
        for (final String name : names) {
            final String fileDst2 = Paths.get(this.rootLocation,this.soilRootDirectory, dir, name).toString();
