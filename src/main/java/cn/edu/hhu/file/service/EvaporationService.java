...
public class EvaporationService {
    ...
    private String evaporationRootDirectory;
    @Value("${spring.servlet.multipart.location}")
    private String rootLocation;

    public List<FileDTO> listFile(final String dir) {
        final List<FileDTO> fileDTOList = new ArrayList<FileDTO>();
        if (this.evaporationRootDirectory.equals(dir)) {
            final String dst = Paths.get(this.rootLocation, this.evaporationRootDirectory, dir).toString();
...
//        fileDTOList.sort(Comparator.comparing((Function<? super Object, ? extends Comparable>)FileDTO::getLastModified).reversed());
    ...
    public InputStream downloadFiles(final String dir, final List<String> names) throws IOException {
        if (names.size() <= 1) {
            final String fileDst = Paths.get(this.rootLocation, dir, names.get(0)).toString();
            final File file = new File(fileDst);
            return new FileInputStream(file);
        }
        final List<File> fileList = new ArrayList<File>();
        for (final String name : names) {
            final String fileDst2 = Paths.get(this.rootLocation, dir, name).toString();
    ...
    private File uplaodHeihaiFile(final MultipartFile file, final String dir) {
        ...
        if (null != file) {
            final String dst = Paths.get(this.rootLocation, dir).toString();
            try {
